...
function build_qtsvg() {
  ...
  pushd "$build_dir"
  qt-configure-module.bat "$_PROJECT_ROOT/assets/qtsvg-$_QT_BRANCH"
  cmake --build . --parallel
  cmake --install .
  popd
}
...
function build_qttools() {
  ...
  pushd "$build_dir"
  qt-configure-module.bat "$_PROJECT_ROOT/assets/qttools-$_QT_BRANCH" \
...
function build_qttranslations() {
  ...
  pushd "$build_dir"
  qt-configure-module.bat "$_PROJECT_ROOT/assets/qttranslations-$_QT_BRANCH"
  cmake --build . --parallel
  cmake --install .
  popd
}
